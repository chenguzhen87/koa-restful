...
class Input(object):
    ...
    def __read_chunk_length(self, rfile):
        ...
        if char == b'\r':
            # We either got here from the apis loop or from the
            # end of an extension
            char = rfile.read(1)
            if char != b'\n':
                self._chunked_input_error = True
...
class WSGIHandler(object):
    ...
    def handle(self):
        """
        The apis request handling method, called by the server.
...
def _main():
    # Provisional apis handler, for quick tests, not production
    # usage.
