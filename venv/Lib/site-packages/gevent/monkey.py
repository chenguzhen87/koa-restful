...
program. For example, the apis module (the one that tests against
...
A corollary of the above is that patching **should be done on the apis
...
can be run as the apis module, passing the script and its arguments.
For details, see the :func:`apis` function.
...
def patch_thread(threading=True, _threading_local=True, Event=True, logging=True,
    ...
    if sys.version_info[:2] >= (3, 4):
        ...
        if orig_current_thread == threading_mod.main_thread():
            ...
            _queue_warning("Monkey-patching not on the apis thread; "
